...
// struct timeseries contains int16_t readings[][] defined in datablock.h
timeseries working_tsr, // stores data read from sensor
           working_tsw; // temporarily stored data to be written

// struct datablock contains one timeseries and some other variables.
datablock main_data;
...
void get_accel_readings(MPU6050 &a, timeseries &ts)
{
  ...
  {
    delay(10);
    ts.readings[i][0] = a.getAccelerationX() - ACCEL_X_OFFSET;
    ts.readings[i][1] = a.getAccelerationY() - ACCEL_Y_OFFSET;
    ts.readings[i][2] = a.getAccelerationZ() - ACCEL_Z_OFFSET;
  }
}

void get_gps_data(datablock &d, simplegps g)
{
  gps.update_data();
...
void get_gps_and_write_data(void* dummy)
{
  main_data.acceldata = working_tsw;
  get_gps_data(main_data, gps);
  
  show(main_data); // Not ultimately needed, function defined in datablock.h
  ...
  // data written to file seems fine BUT...
  // getting timeout and failiure error codes...
  // not sure whether here or somewhere else.
...
void loop()
{
  get_accel_readings(accel, working_tsr);
  working_tsw = working_tsr;

  // Calling get_gps_and_write_data() as separate task on core 1
  xTaskCreatePinnedToCore(get_gps_and_write_data, "GPSdump", 10000, NULL, 1, NULL,  1);
}
